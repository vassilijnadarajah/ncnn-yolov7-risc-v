...
void YoloV7::nms_sorted_bboxes(const std::vector<Object>& faceobjects, std::vector<int>& picked, float nms_threshold, bool agnostic)
{
    picked.clear();
...
void YoloV7::generate_proposals(const ncnn::Mat& anchors, int stride, const ncnn::Mat& in_pad, const ncnn::Mat& feat_blob, float prob_threshold, std::vector<Object>& objects)
{
...
int YoloV7::detect(const cv::Mat& bgr, std::vector<Object>& objects)
{
    ncnn::Net yolov7;
    ...
    if(yolov7.load_param(YoloV7::path_to_param))
        exit(-1);
    if(yolov7.load_model(YoloV7::path_to_bin))
        exit(-1);
